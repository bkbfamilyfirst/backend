...
exports.getReports = async (req, res) => {
  try {
    ...
    // Total keys generated by retailer
    const totalKeys = await Key.countDocuments({ generatedBy: retailerId });

    // Total balance (keys assigned to retailer but not transferred)
    const totalBalance = await Key.countDocuments({ assignedTo: retailerId, isAssigned: false });

    // Total transferred (keys transferred from retailer to parent)
    const totalTransferred = await KeyTransferLog.countDocuments({ from: retailerId, transferType: 'retailer-to-parent' });
...
exports.getRetailerStats = async (req, res) => {
  try {
    const retailerId = req.user.id;
    // Count keys generated by this retailer
    const totalKeys = await Key.countDocuments({ generatedBy: retailerId });
    ...
    const totalTransfers = await KeyTransferLog.countDocuments({ fromUser: retailerId });
